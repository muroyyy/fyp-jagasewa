...
import { Bell, Search, Filter, Clock, CheckCircle, Eye, EyeOff, Wrench, DollarSign, AlertCircle, FileText, Calendar, Building2, ChevronLeft, ChevronRight, RefreshCw, ArrowLeft } from 'lucide-react';
...
export default function LandlordNotifications() {
  ...
  const fetchNotifications = async () => {
    ...
    try {
      ...
      const response = await fetch(`${API_BASE_URL}/api/landlord/notifications.php?all=true`, {
        method: 'GET',
        headers: {
      ...
      if (result.success) {
        ...
        const readNotifications = JSON.parse(localStorage.getItem('landlord_read_notifications') || '[]');
  ...
  const markAsRead = (notificationId) => {
    ...
    if (!readNotifications.includes(notificationId)) {
      ...
      localStorage.setItem('landlord_read_notifications', JSON.stringify(readNotifications));
  ...
  const markAllAsRead = () => {
    ...
    localStorage.setItem('landlord_read_notifications', JSON.stringify(allIds));
  ...
  const stats = {
    ...
    unread: unreadCount,
    maintenance: notifications.filter(n => n.type === 'maintenance_request').length,
    payments: notifications.filter(n => n.type === 'rent_payment' || n.type === 'payment_overdue').length
  };
            ...
            <div className="flex items-center space-x-3">
              ...
              {unreadCount > 0 && (
                <button
                  ...
                  className="flex items-center space-x-2 px-4 py-2 bg-blue-600 text-white rounded-xl hover:bg-blue-700 transition-colors cursor-pointer"
                >
            ...
            <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
              ...
              <div className="flex items-center space-x-3">
                ...
                >
                  ...
                  <option value="rent_payment">Payment Received</option>
                  <option value="payment_overdue">Overdue Payment</option>
                </select>
              ...
              <div className="divide-y divide-gray-100">
                {paginatedNotifications.map((notification) => (
                  <div
                    ...
                    className={`p-6 border-l-4 hover:bg-gray-50 transition-colors cursor-pointer ${getPriorityColor(notification.priority)} ${!notification.is_read ? 'bg-blue-50/50' : ''}`}
                  >
