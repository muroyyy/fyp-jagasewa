...
export default function LandlordEditTenant() {
  ...
  const fetchTenantData = async () => {
    try {
      ...
      const response = await fetch(`${import.meta.env.VITE_API_URL}/api/landlord/tenant-details.php?tenant_id=${id}`, {
        headers: {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const token = localStorage.getItem('session_token');
      const response = await fetch(`${import.meta.env.VITE_API_URL}/api/landlord/update-tenant.php`, {
        method: 'POST',
        headers: {
        ...
        <form onSubmit={handleSubmit} className="bg-white rounded-2xl shadow-lg border border-gray-100 p-6">
          ...
          <div className="flex flex-col sm:flex-row justify-end gap-4 mt-8 pt-6 border-t border-gray-200">
            ...
            <button
              type="submit"
              disabled={saving}
              className="px-6 py-3 bg-blue-600 text-white rounded-lg hover:shadow-lg font-medium transition-all flex items-center justify-center disabled:opacity-50 disabled:cursor-not-allowed cursor-pointer"
            >
              {saving ? (
                <>
