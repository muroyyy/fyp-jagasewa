...
export default function LandlordDocuments() {
  ...
  const fetchDocuments = async () => {
    try {
      ...
      let url = 'http://localhost:8000/api/landlord/documents.php';
  ...
  const handleUpload = async (e) => {
    ...
    try {
      ...
      const response = await fetch('http://localhost:8000/api/landlord/upload-document.php', {
        method: 'POST',
        headers: {
          'Authorization': `Bearer ${token}`
        },
  ...
  const handleDelete = async (documentId) => {
    ...
    try {
      const token = localStorage.getItem('session_token');
      const response = await fetch(`http://localhost:8000/api/landlord/delete-document.php?document_id=${documentId}`, {
        method: 'DELETE',
        headers: {
          'Authorization': `Bearer ${token}`
        }
